...
let createUserRds = (origin, options) => {
  ...
  let userUid = options.userUid || `${config.redis.userPrefix.index}${getUuid()}`,
      userEmail = options.userEmail || `${config.redis.userPrefix.index}${config.redis.userPrefix.email}${origin.email}`,
      userToken = options.userToken || `${config.redis.userPrefix.index}${config.redis.userPrefix.token}${origin.token}`
      // 后期还会增加
