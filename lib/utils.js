          ...
          if (response.data.result) {
            ...
            return tokenSDKServer.backupData(didttm.did, key, type, list, signStr).then(response => {
              if (response.data.result) {
                ...
                return Promise.reject({status: false, error: new Error('备份父did任务列表失败')})
              }
            })
          } else {
            return Promise.reject({status: false, error: new Error('拉取父did任务列表失败')})
          }
        })
      })
      .catch(errorObj => {
