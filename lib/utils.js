...
let retReceiptMsg = (msgObj) => {
  let reObj = websocketLocal.createMessage({
    messageId: msgObj.messageId,
    claim_sn: msgObj.content.applicationCertificateDataBean.claim_sn
...
let opPendintTaskAdidIdentity = (claim_sn, value) => {
...
let opPendingTaskItem = (key, taskItem) => {
  switch (taskItem.type) {
    case 'adidIdentityConfirm':
      opPendintTaskAdidIdentity(key, taskItem)
      break
    default:
      break
  }
