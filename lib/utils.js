...
let opConfirmAdidIdentity = (msgObj) => {
  ...
  .catch(({isError, payload}) => {
    ...
    if (isError) {
      ...
      websocketLocal.websocketClient.send(websocketLocal.createMessage(payload, [msgObj.sender], 'error', getUuid()))
    } else {
      // 发消息
      ...
      websocketLocal.websocketClient.send(websocketLocal.createMessage(payload, [msgObj.sender], 'pending', getUuid()))
