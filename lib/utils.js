...
let addPendintTaskAdidIdentity = (msgObj) => {
  ...
  if (!applicationCertificateDataBean.claim_sn || !applicationCertificateDataBean.templateId || !applicationCertificateDataBean.ocrData.name || !applicationCertificateDataBean.ocrData.hostname || !applicationCertificateDataBean.ocrData.port || !applicationCertificateDataBean.ocrData.path) {
    websocketLocal.websocketClient.send(websocketLocal.createMessage({msgContentType: 'adidRandomCode', error: config.errorMap.arguments.message}, [msgObj.sender], 'error', getUuid()))
    return
  }
  // 检查是否签过名
  let pvdata = null
  ...
  .catch(({isError, payload}) => {
    ...
    if (isError) {
      ...
      websocketLocal.websocketClient.send(websocketLocal.createMessage({msgContentType: 'adidRandomCode', error: payload}, [msgObj.sender], 'error', getUuid()))
    } else {
      // 发消息
      ...
      websocketLocal.websocketClient.send(websocketLocal.createMessage({msgContentType: 'adidRandomCode', randomCode: payload}, [msgObj.sender], 'pending', getUuid()))
...
let opPendintTaskAdidIdentity = (claim_sn, value) => {
  ...
  url = 'lixiaodan.org:3000/path/randomCode.ttm'
  return axios({
    url : url,
    method: 'get'
  })
