 ...
 */
let createUserRds = (origin) => {
  // 检查参数是否有效
  if (!origin.email && !origin.token) {
    return Promise.resolve({error: new Error(`${config.errorMap.createUserRdsArg.message}`), result: null})
  }
  let user = tokenSDKServer.utils.schemeToObj(config.redis.userScheme, origin) // 这个方法里没有散列password
  let userUid = `${config.redis.userPrefix.index}${utils.getUuid()}`,
      userEmail = `${config.redis.userPrefix.index}${config.redis.userPrefix.email}${origin.email}`,
      userToken = `${config.redis.userPrefix.index}${config.redis.userPrefix.token}${origin.token}`
      // 后期还会增加
  return redisUtils.str.set(userUid, user).then(({error, result}) => {
    if (error) {
  ...
  .catch(({isError, payload}) => {
    if (isError) {
      return Promise.resolve({errro: payload, result: null})
    } else {
      return Promise.resolve({errro: null, result: payload})
    }
  })
}
...
let getUserByEmailRds = (email) => {
  // 先从redis里获取uid
  return redisUtils.str.get(`${config.redis.userPrefix.index}${config.redis.userPrefix.email}`).then(({error, result}) => {
    // result 就是查询到的uid
    if (error) {
      return Promise.reject({isError: true, payload: error})
    } else {
...
let getUserByTokenRds = (token) => {
  return redisUtils.str.get(`${config.redis.userPrefix.index}${config.redis.userPrefix.token}`).then(({error, result}) => {
    if (error) {
      return Promise.reject({isError: true, payload: error})
    } else {
      // return result
