...
// 发送receipt消息
// let retReceiptMsg = (msgObj) => {
//   let msgId = msgObj.messageId || ''
//   let claim_sn = msgObj.content.applicationCertificateDataBean.claim_sn || ''
//   let reObj = ''
//   if (!msgId || !claim_sn) {
//     reObj = websocketLocal.createMessage({
//       messageId: msgId,
//       claim_sn: claim_sn
//     }, [msgObj.sender], 'receipt', getUuid())
//   } else {
//     reObj = websocketLocal.createMessage({
//       messageId: msgId,
//       claim_sn: claim_sn
//     }, [msgObj.sender], 'error', getUuid())
//   }
//   // console.log('发出去receipt', reObj)
//   websocketLocal.websocketClient.send(reObj)
// }
...
let opBusinessLicenseConfirm = (claim_sn, value) => {
  // console.log('opBusinessLicenseConfirm')
  ...
  let isPersonCheck = pendingTask[claim_sn].isPersonCheck
  // console.log('value', value)
  // 是否父签名
  if (!pendingTask[claim_sn].isPdidCheck) {
    tokenSDKServer.hasValidSign(claim_sn, didttm.did).then(response => { // response: {error, result}
      // console.log(response)
      if (!response.error && response.result && !pendingTask[claim_sn].isPdidCheck) {
        pendingTask[claim_sn].isPdidCheck = true
        tokenSDKServer.setPvData(pvdata, {needEncrypt: true})
      }
    })
  }
  // 签名
  if (pendingTask[claim_sn].isPersonCheck && pendingTask[claim_sn].isPdidCheck) {
  // if (true) { // 测试用
    // function signCertify(did, claim_sn, name, templateId, hashValue, explain, expire, sign) {
    // tokenSDKServer.signCertify(didttm.did, claim_sn, didttm.nickname, value.content.businessLicenseData.templateId, hashValue, explain, expire, sign)
    ...
    tokenSDKServer.signCertify(claim_sn, '认证应用签名营业执行', date).then(response => { // response: {error, result}
      // console.log('认证应用签名营业执行', response)
      if (response.error) {
    ...
    // 发消息
    .catch(({isError, payload}) => {
      // console.log('发消息', isError, payload)
      if (isError) {
        tokenSDKServer.send({type: 'error', message: '', error: payload}, [value.sender], 'confirm')
      } else {
        tokenSDKServer.send({type: 'finish', message: '', data: payload}, [value.sender], 'confirm')
      }
      return true
    })
    // 在pendingTask里删除，在certifies时增加。
    .then(bool => {
      tokenSDKServer.movePendinTaskToCertifies(claim_sn).then(response => { // response: {error, result}
        // console.log('response', response)
        if (response.error) {
          console.log(result)
          // console.log('response', response)
        } else {
          return Promise.reject({error: response.error, result: null})
        }
      }).catch(({error, result}) => {
...
//   let applicationCertificateDataBean = value.msgObj.content.applicationCertificateDataBean
//   let url = `${applicationCertificateDataBean.ocrData.hostname}:${applicationCertificateDataBean.ocrData.port}${applicationCertificateDataBean.ocrData.path}/randomCode.ttm`
//   // url = 'http://lixiaodan.org:9800/test/randomCode' // ok
//   // url = 'http://lixiaodan.org:9800/path/randomCode.ttm' // ok
//   let hashValue = ''
//   let pvdata = ''
//   // 若超过24h，则删除pendingTaskItem
//   let now = new Date().getTime()
//   if (now - value.createTime > config.timeInterval.delPendingTaskAdidCert) {
//     return tokenSDKServer.getPvData(didttm.did).then(response => {
//       if (response.data.result) {
//         let pvdataCt = response.data.result.data
//         pvdata = tokenSDKServer.decryptPvData(pvdataCt, priStr)
//         pvdata = JSON.parse(pvdata)
//         let pendingTask = pvdata.pendingTask || {}
...
//     .catch(({isError, payload}) => {
//         websocketLocal.websocketClient.send(websocketLocal.createMessage({
//           messageId: value.msgObj.messageId,
//           error: payload,
//           claim_sn: applicationCertificateDataBean.claim_sn
//         }, [value.msgObj.sender], 'pendTimeout', getUuid()))
//         return {isError: isError, payload: payload}
//       // if (isError) {
//       // } else {
//       //   websocketLocal.websocketClient.send(websocketLocal.createMessage({
...
//     // return true // 用于测试
//     if (response.data === value.randomCode) {
//       return true
//     } else {
//       return Promise.reject({isError: true, payload: new Error(config.errorMap.randomCode.message)})
//     }
//   })
//   // 比对hashValue
//   .then(bool => {
...
//       console.log('比对hashValue', response)
//       if (response.result) {
//         hashValue = response.hashValueChain
//         return true
//       } else {
//         return Promise.reject({isError: true, payload: new Error(config.errorMap.hashValue.message)})
//       }
//     })
//   })
//   // 签名
...
//     let signObj = `claim_sn=${applicationCertificateDataBean.claim_sn},templateId=${applicationCertificateDataBean.templateId},hashCont=${hashValue},did=${didttm.did},name=${didttm.nickname},explain=${explain},expire=${expire}`
//     let sign = tokenSDKServer.signEcdsa(signObj, priStr)
//     return tokenSDKServer.signCertify(didttm.did, applicationCertificateDataBean.claim_sn, didttm.nickname, applicationCertificateDataBean.templateId, hashValue, explain, expire, sign).then(response => {
//       // console.log('signCertify', response.data)
//       if (response.data.error) {
//         return Promise.reject({isError: true, payload: new Error(response.data.error.message || config.errorMap.sign.message)})
//       } else {
//         return true
//       }
//     })
//   })
//   // 删除pvdata.pendingTask里的事项
//   // 备份pvdata
//   .then(bool => {
//     // return true // 用于测试
//     return tokenSDKServer.getPvData(didttm.did).then(response => {
...
//         pvdata = JSON.parse(pvdata)
//         let pendingTask = pvdata.pendingTask || {}
//         delete pendingTask[applicationCertificateDataBean.claim_sn]
//         pvdata.pendingTask = pendingTask
//         pvdataCt = tokenSDKServer.encryptPvData(pvdata, priStr)
//         return updataPvdata(pvdataCt, didttm.did, priStr)
//       } else {
...
//         error: payload,
//         claim_sn: applicationCertificateDataBean.claim_sn
//       }, [value.msgObj.sender], 'error', getUuid()))
//       return {isError: isError, payload: payload}
//     } else {
//       websocketLocal.websocketClient.send(websocketLocal.createMessage({
//         messageId: value.msgObj.messageId,
//         claim_sn: applicationCertificateDataBean.claim_sn
//       }, [value.msgObj.sender], 'receipt', getUuid()))
//       return {isError: isError, payload: payload}
//     }
//   })
// }
let opPendingTaskItem = (key, taskItem) => {
  // console.log(key, taskItem)
  switch (taskItem.type) {
    case 'businessLicenseConfirm':
      opBusinessLicenseConfirm(key, taskItem)
      break
